...
NOMBRE_CONEXION_DESTINO="DESARROLLO"
...
ARCHIVOS_PATH_PENTAHO="/home/marcosd94/ETL-portal/pentaho/Jobs/"
...
PENTAHO_JOB_CARGA_FUN=`echo $ARCHIVOS_PATH_PENTAHO'CARGAR_DETALLES_FUNCIONARIOS.kjb'`
PENTAHO_JOB_JSON_FUN=`echo $ARCHIVOS_PATH_PENTAHO'GENERAR_JSON_FUNCIONARIOS.kjb'`
...
LOG_PATH="/home/marcosd94/ETL-portal/pentaho/prueba/Log/"
...
BACKUP_PATH="/home/marcosd94/ETL-portal/pentaho/prueba/Backup/"
BACKUP_NAME="backup_$(date +%Y_%m_%d)"
BACKUP_FILE=`echo $BACKUP_PATH$BACKUP_NAME'.bak'`

estado=$(psql -qtA -U $USUARIO_BD_DESTINO -h $IP_BD_DESTINO -p $PUERTO_BD_DESTINO $NOMBRE_BD_DESTINO -w -c 'select estado from opendata.etl_procesamiento where fecha_inicio=(select max(fecha_inicio) from opendata.etl_procesamiento)')

if [ $estado != procesando ]; then

	psql -U $USUARIO_BD_DESTINO -W -h $IP_BD_DESTINO -p $PUERTO_BD_DESTINO $NOMBRE_BD_DESTINO -w -c 'insert into opendata.etl_procesamiento values ('"'funcionarios'"','"'procesando'"', current_timestamp, null)'
	...
	psql -U $USUARIO_BD_DESTINO -W -h $IP_BD_DESTINO -p $PUERTO_BD_DESTINO -w -c 'DROP DATABASE IF EXISTS '$NOMBRE_BD_PENTAHO
	psql -U $USUARIO_BD_DESTINO -W -h $IP_BD_DESTINO -p $PUERTO_BD_DESTINO -w -c 'CREATE DATABASE '$NOMBRE_BD_PENTAHO
	psql -U $USUARIO_BD_DESTINO -W -h $IP_BD_DESTINO -p $PUERTO_BD_DESTINO -d $NOMBRE_BD_PENTAHO -w -c 'CREATE EXTENSION unaccent SCHEMA pg_catalog;ALTER FUNCTION unaccent(text) IMMUTABLE;'
	...
	if [ -f $PENTAHO_JOB_CARGA_VENCIMIENTO ]; then
		...
		sh kitchen.sh -file $PENTAHO_JOB_CARGA_VENCIMIENTO -param:nombreConexionOrigen=$NOMBRE_CONEXION_ORIGEN -param:nombreBDFallida=$NOMBRE_BD_DESTINO -param:ipBDOrigen=$IP_BD_ORIGEN -param:puertoBDOrigen=$PUERTO_BD_ORIGEN -param:nombreBDOrigen=$NOMBRE_BD_ORIGEN -param:usuarioBDOrigen=$USUARIO_BD_ORIGEN -param:passBDOrigen=$PASS_BD_ORIGEN -param:nombreConexionDestino=$NOMBRE_CONEXION_DESTINO -param:ipBDDestino=$IP_BD_DESTINO -param:puertoBDDestino=$PUERTO_BD_DESTINO -param:nombreBDDestino=$NOMBRE_BD_PENTAHO -param:usuarioBDDestino=$USUARIO_BD_DESTINO -param:passBDDestino=$PASS_BD_DESTINO  level=Debug >> $LOG_FILE
		error=`grep ETL_PORTAL_SFP $LOG_FILE`
		if [[ $error == *ETL_PORTAL_SFP* ]]; then
			exit
		fi
...
#Generacion de json de OEE
	...
	if [ -f "$PENTAHO_JOB_CSV_OEE" ]; then
		sh kitchen.sh -file $PENTAHO_JOB_CSV_OEE -param:nombreConexion=$NOMBRE_CONEXION_DESTINO -param:ipBD=$IP_BD_DESTINO -param:puertoBD=$PUERTO_BD_DESTINO -param:nombreBD=$NOMBRE_BD_PENTAHO -param:usuarioBD=$USUARIO_BD_DESTINO -param:passBD=$PASS_BD_DESTINO -param:nombreBDFallida=$NOMBRE_BD_DESTINO -param:csvFile=$CSV_FILE -param:correoDestinatario=$CORREO_DESTINATARIO -param:correoEmisor=$CORREO_EMISOR -param:nombreDestinatario=$NOMBRE_DESTINATARIO -param:puertoMail=$PUERTO_SERVER -param:smtpServer=$SMTP_SERVER level=Debug >> $LOG_FILE
	...
	for i in `seq 06`; do
		...
		MES=`date -d $dat "-$i months" +'%m'`
		ANHO=`date -d $dat "-$i months" +'%Y'`
		...
		psql -U $USUARIO_BD_DESTINO -W -h $IP_BD_DESTINO -p $PUERTO_BD_DESTINO $NOMBRE_BD_PENTAHO -w -c 'drop table if exists opendata.detalles_funcionarios_'$ANHO'_'$MES' cascade'

		#Borra el mes de la base de datos destino
		psql -U $USUARIO_BD_DESTINO -W -h $IP_BD_DESTINO -p $PUERTO_BD_DESTINO $NOMBRE_BD_PENTAHO -w -c 'drop table if exists opendata.funcionarios_'$ANHO'_'$MES' cascade'

		if [ -f $PENTAHO_JOB_CARGA_FUN ]; then
			#Carga el detalle de funcionarios del mes
			sh kitchen.sh -file $PENTAHO_JOB_CARGA_FUN -param:anho=$ANHO -param:mes=$MES -param:nombreConexionOrigen=$NOMBRE_CONEXION_ORIGEN -param:ipBDOrigen=$IP_BD_ORIGEN -param:puertoBDOrigen=$PUERTO_BD_ORIGEN -param:nombreBDOrigen=$NOMBRE_BD_ORIGEN -param:usuarioBDOrigen=$USUARIO_BD_ORIGEN -param:passBDOrigen=$PASS_BD_ORIGEN -param:nombreBDFallida=$NOMBRE_BD_DESTINO -param:nombreConexionDestino=$NOMBRE_CONEXION_DESTINO -param:ipBDDestino=$IP_BD_DESTINO -param:puertoBDDestino=$PUERTO_BD_DESTINO -param:nombreBDDestino=$NOMBRE_BD_PENTAHO -param:usuarioBDDestino=$USUARIO_BD_DESTINO -param:passBDDestino=$PASS_BD_DESTINO -param:correoDestinatario=$CORREO_DESTINATARIO -param:correoEmisor=$CORREO_EMISOR -param:nombreDestinatario=$NOMBRE_DESTINATARIO -param:puertoMail=$PUERTO_SERVER -param:smtpServer=$SMTP_SERVER level=Debug >> $LOG_FILE
			error=`grep ETL_PORTAL_SFP $LOG_FILE`
			if [[ $error == *ETL_PORTAL_SFP* ]]; then
				cd $ARCHIVOS_PATH
				rm *.nuevo
		...
		if [ -f $PENTAHO_JOB_CSV_FUN ]; then
			sh kitchen.sh -file $PENTAHO_JOB_CSV_FUN -param:anho=$ANHO -param:mes=$MES -param:nombreConexion=$NOMBRE_CONEXION_DESTINO -param:ipBD=$IP_BD_DESTINO -param:puertoBD=$PUERTO_BD_DESTINO -param:nombreBD=$NOMBRE_BD_PENTAHO -param:usuarioBD=$USUARIO_BD_DESTINO -param:passBD=$PASS_BD_DESTINO -param:nombreBDFallida=$NOMBRE_BD_DESTINO -param:csvFile=$CSV -param:correoDestinatario=$CORREO_DESTINATARIO -param:correoEmisor=$CORREO_EMISOR -param:nombreDestinatario=$NOMBRE_DESTINATARIO -param:puertoMail=$PUERTO_SERVER -param:smtpServer=$SMTP_SERVER level=Debug >> $LOG_FILE
		...
		#Zipea los archivos
		zip -j `echo $LOG_PATH$LOG_NAME'_'$ANHO'-'$MES'.zip'` $LOG_FILE
	...
	if [ -f $PENTAHO_JOB_CARGA_CONCURSOS ]; then
		sh kitchen.sh -file $PENTAHO_JOB_CONCURSOS -param:nombreConexionOrigen=$NOMBRE_CONEXION_ORIGEN -param:nombreBDFallida=$NOMBRE_BD_DESTINO -param:ipBDOrigen=$IP_BD_ORIGEN -param:puertoBDOrigen=$PUERTO_BD_ORIGEN -param:nombreBDOrigen=$NOMBRE_BD_ORIGEN -param:usuarioBDOrigen=$USUARIO_BD_ORIGEN -param:passBDOrigen=$PASS_BD_ORIGEN -param:nombreConexionDestino=$NOMBRE_CONEXION_DESTINO -param:ipBDDestino=$IP_BD_DESTINO -param:puertoBDDestino=$PUERTO_BD_DESTINO -param:nombreBDDestino=$NOMBRE_BD_PENTAHO -param:usuarioBDDestino=$USUARIO_BD_DESTINO -param:passBDDestino=$PASS_BD_DESTINO  -param:correoDestinatario=$CORREO_DESTINATARIO -param:correoEmisor=$CORREO_EMISOR -param:nombreDestinatario=$NOMBRE_DESTINATARIO -param:puertoMail=$PUERTO_SERVER -param:smtpServer=$SMTP_SERVER level=Debug >> $LOG_FILE
		error=`grep ETL_PORTAL_SFP $LOG_FILE`
		if [[ $error == *ETL_PORTAL_SFP* ]]; then
			cd $ARCHIVOS_PATH
			rm *.nuevo
	...
	if [ -f $PENTAHO_JOB_JSON_CONCURSOS ]; then
		sh kitchen.sh -file $PENTAHO_JOB_JSON_CONCURSOS -param:nombreBDFallida=$NOMBRE_BD_DESTINO -param:nombreConexion=$NOMBRE_CONEXION_DESTINO -param:ipBD=$IP_BD_DESTINO -param:puertoBD=$PUERTO_BD_DESTINO -param:nombreBD=$NOMBRE_BD_DESTINO -param:usuarioBD=$USUARIO_BD_DESTINO -param:passBD=$PASS_BD_DESTINO -param:jsonFile=$JSON_FILE  -param:correoDestinatario=$CORREO_DESTINATARIO -param:correoEmisor=$CORREO_EMISOR -param:nombreDestinatario=$NOMBRE_DESTINATARIO -param:puertoMail=$PUERTO_SERVER -param:smtpServer=$SMTP_SERVER level=Debug >> $LOG_FILE
